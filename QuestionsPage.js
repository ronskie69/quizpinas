...
import { laugh, applause, bgSound } from './sounds.js'

function QuestionsPage({ onFinishQuiz, questions }) {
    ...
    const bgSounds = useRef(new Audio(bgSound.url))
    bgSounds.current.loop = true;

    const onClickNext = (ans) => {
        if(ans){
    ...
    return (
        <div className ="card card-question" style = {{animation: wrong ? 'animateError .3s infinite alternate' : ''}}>
            <h3>Question No. {currentQuestion + 1} out of {questionLength}</h3>
        {
            explanation ?
        ...
        <div className ="question-box">
            ...
            <div className ="answers-holder">
            {
                questions[currentQuestion].answers.map((answers) =>{
                    return (
                        <button key = {Math.random()} className ="button-answers" onClick = {() => onClickNext(answers.isCorrect)}>
                            {answers.answer}
                        </button>
                    )
                })
